...
void main() {
  ...
  //
  // In integer division, when both operands are positive, the result is
  // also always positive.
  //
  // Different from regular division, integer division always returns an
  // {Integer type} independently of the scenario, if the value is too
  // large for an {Integer type} to store, then an error will be thrown.
  //
  // Obs: there are no tests for [unconstrained] [fractional arbitrary types]
  // because it is impossible. An [arbitrary precision type] can not be [unconstrained]
  // and [fractional] at the same time, if such a thing is attempted, the {value}
  // will be a [float] rather than a [fractional]:
  //
  // Numeric(value: '0.35', precision: 2, scale: 2) -> Fractional
  // Numeric(value: '3.14', precision: 3, scale: 2) -> Float
  // Numeric(value: '0.13') -> Float, because '0' is treated as significant digit
  //
  // A similar thing applies to [arbitrary precision types with negative scale],
  // because to define them you need to specify a {scale}, therefore being
  // [unconstrained] is impossible:
  //
  ...
  //
  // In division, when operands are of distinct signs, the result
  // is always negative.
  //
  // Different from regular division, integer division always returns an
  // {Integer type} independently of the scenario, if the value is too
  // large for an {Integer type} to store, then an error will be thrown.
  //
  // Obs: in this test group, there are no tests for [Serial types], as
  // their minimum {value} is 1, therefore not being able to store negative
  // values.
  //
  // As well as there are no tests for [fractional arbitrary precision types],
  // because they represent a value between 0 and 1, and zero values cannot
  // receive a sign.
  //
  ...
  // Tests if integer division where both operands are integer can
  // yield a whole result correctly.
  //
  // In integer division, an operation always results in a whole
  // number. In this group, only the scenarios where the results
  // are exact and have no decimal part are tested, then it works
  ...
  // large for an {Integer type} to store, then an error will be thrown.
  //
  // Obs: in this group, only [int arbitrary precision types] and
  // [Integer types] are tested.
  ...
  // Tests if integer division where both operands are integer can yield
  // a fractional result correctly.
  //
  // In integer division, an operation always results in a whole number.
  // In this group, all the operations result in a value that has a
  // decimal part, so unlike regular division, the final result is
  // truncated:
  //
  ...
  // Tests if integer division where the first operand is of
  // [Integer type] and the second one is an [integer-valued]
  // [floating point type] can yield a whole result correctly.
  //
  // In integer division, an operation always results in a whole
  // number. In this group, only the scenarios where the results
  // are exact and have no decimal part are tested, then it works
  ...
  // large for an {Integer type} to store, then an error will be thrown.
  //
  // Obs: in this group, only [float arbitrary precision types] and
  // [floating point types] are tested.
  ...
  // Tests if integer division where the first operand is of [Integer type]
  // and the second one is an [integer-valued] [floating point type] can
  // yield a fractional result correctly.
  //
  // In integer division, an operation always results in a whole number.
  // In this group, all the operations result in a value that has a
  // decimal part, so unlike regular division, the final result is
  ...
  // Tests if integer division where the first operand is of
  // [Integer type] and the second one is a [floating point type]
  // with decimal part can yield an exact result correctly.
  //
  // In integer division, an operation always results in a whole
  // number. In this group, only the scenarios where the results
  // are exact and have no decimal part are tested, then it works
  ...
  // Tests if integer division where the first operand is of
  // [Integer type] and the second one is a [floating point type]
  // with decimal part can yield a fractional result correctly.
  //
  // In integer division, an operation always results in a whole number.
  // In this group, all the operations result in a value that has a
  // decimal part, so unlike regular division, the final result is
  ...
  // Tests if integer division where the first operand is of
  // [Integer type] and the second one is a [floating point type]
  // greater than 0 and less than 1, representing only the
  // fractional part, can yield an exact result correctly.
  //
  // In integer division, an operation always results in a whole
  // number. In this group, only the scenarios where the results
  // are exact and have no decimal part are tested, then it works
  ...
  // large for an {Integer type} to store, then an error will be thrown.
  //
  // Obs: in this group, only [fractional arbitrary precision types] and
  // [floating point types] with fractional only values are tested.
  group('fractional only floating point division (exact)', () {
    test('should correctly divide double primitive', () {
      final value1 = SmallSerial(4);
      const value2 = 0.5;
  ...
  // Tests if integer division where the first operand is of
  // [Integer type] and the second one is a [floating point type]
  // greater than 0 and less than 1, representing only the fractional
  // part, can yield an fractional result correctly.
  //
  // In integer division, an operation always results in a whole number.
  // In this group, all the operations result in a value that has a
  // decimal part, so unlike regular division, the final result is
  ...
  //
  // In integer division, any number divided by zero results in
  // undefined. For example:
  //
  // 10 / 0 = ? -> Undefined
  //
  // So in programming languages we treat this by throwing an error
  // when division by zero is attempted.
  //
  // Obs: There are no tests for [fractional floating point],
  // [fractional arbitrary types] because they can't represent
  // a zero value in meaningful way.
  //
  // As well as there are no tests for [Serial types] because
  // their minimum {value} is 1, therefore they can't store
  // 0 as {value}.
  ...
  // 20 / 1 = 20
  //
  // Obs: There are no tests for [fractional floating point] as
  // they can't represent an [Integer value]. Similarly there are
  // no tests for [fractional arbitrary types] because they can't
  ...
  //
  // In integer division, any value divided by itself results in
  // one. For example:
  //
  // 20 / 20 = 1
  //
  ...
  // types will cast correctly both operands and return the final
  // result of the expected type.
  //
  // This group of tests ensures that type casting is working
  // correctly when operands are of different types.
  //
  // The focus of the tests is on the final type rather than the
  // value. division resulting values are tested in other
  // groups within this file.
  //
  // Obs: 1 is used as {value} in most of the operations instead
  // of 0 due to the minimum {value} of [Serial types] that is 1.
  group('type casting', () {
    ...
    test('should return BigSerial as result when dividing BigSerial', () {
      final value1 = SmallSerial(1);
      final value2 = BigSerial(1);
      final operation = value1 ~/ value2;
    ...
    test('should return BigSerial as result when dividing Serial', () {
      final value1 = SmallSerial(1);
      final value2 = Serial(1);
      final operation = value1 ~/ value2;
    ...
    test('should return SmallSerial as result when dividing SmallSerial', () {
      final value1 = SmallSerial(1);
      final value2 = SmallSerial(1);
      final operation = value1 ~/ value2;
    ...
    test('should return SmallSerial as result when dividing int primitive', () {
      final value1 = SmallSerial(1);
      const value2 = 1;
      final operation = value1 ~/ value2;
      expect(operation, isA<SmallSerial>());
