...
// is 1, and they cannot store zero values.
  group('positive result tests (positive + negative)', () {
    ...
    test('should return positive value when subtracting SmallInteger', () {
      final value1 = SmallSerial(40);
      final value2 = SmallInteger(-30);
      final expected = SmallInteger(70);
      final operation = value1 - value2;
      expect(operation.identicalTo(expected), true);
    });
...
// Tests if subtraction correctly promotes a less precise value to a
// more precise value when necessary.
//
// This group of tests validates proper type promotion when the second
// operand is more precise than the first. The need for these tests arose
// from a subtle bug found during the development of data types for this mock,
// specifically concerning range adaptation for new values resulting from
// subtraction operations. A typical example is as follows:
//
// SmallInteger(SmallInteger.minValue) - BigInteger(20)
//
// In the example above, the expected behavior is for the {SmallInteger}
// type to be promoted to a {BigInteger} to accommodate the result after
// subtraction, aligning with type casting rules. Previously, this operation
// erroneously triggered a RangeError indicating that {SmallInteger} was
// out of range. This error occurred because the result was assessed against
// the range of the first operand rather than that of the more precise operand.
// This test group specifically addresses and verifies the correction of this issue.
//
// Observations:
// - There are no tests for types with equal or lesser precision than those
// tested in this file.
  group('operation type promotion', () {
    test(
        'should promote correctly when subtracting BigInteger close to min value',
        () {
      final value1 = SmallSerial(SmallSerial.minValue);
      final value2 = BigInteger(20);
      final operation = value1 - value2;
      final expected = BigInteger(SmallSerial.minValue - 20);
      expect(operation.identicalTo(expected), true);
    });

    test(
        'should promote correctly when subtracting BigInteger close to max value',
        () {
      final value1 = SmallSerial(SmallSerial.maxValue);
      final value2 = BigInteger(-20);
      final operation = value1 - value2;
      final expected = BigInteger(SmallSerial.maxValue - (-20));
      expect(operation.identicalTo(expected), true);
    });

    test('should promote correctly when subtracting Integer close to min value',
        () {
      final value1 = SmallSerial(SmallSerial.minValue);
      final value2 = Integer(20);
      final operation = value1 - value2;
      final expected = Integer(SmallSerial.minValue - 20);
      expect(operation.identicalTo(expected), true);
    });

    test('should promote correctly when subtracting Integer close to max value',
        () {
      final value1 = SmallSerial(SmallSerial.maxValue);
      final value2 = Integer(-20);
      final operation = value1 - value2;
      final expected = Integer(SmallSerial.maxValue - (-20));
      expect(operation.identicalTo(expected), true);
    });

    test(
        'should promote correctly when subtracting SmallInteger close to min value',
        () {
      final value1 = SmallSerial(SmallSerial.minValue);
      final value2 = SmallInteger(20);
      final operation = value1 - value2;
      final expected = SmallInteger(SmallSerial.minValue - 20);
      expect(operation.identicalTo(expected), true);
    });
  });
...
// the minimum value constraint of [Serial types], which is 1.
  group('type casting', () {
    ...
    test('should return Integer as result when subtracting Integer', () {
      final value1 = SmallSerial(1);
    ...
    test('should return Integer as result when subtracting negative Integer',
        () {
      final value1 = SmallSerial(1);
      final value2 = Integer(-1);
      final operation = value1 - value2;
    ...
    test('should return SmallInteger as result when subtracting SmallInteger',
        () {
      final value1 = SmallSerial(1);
    ...
    test(
        'should return SmallInteger as result when subtracting negative SmallInteger',
        () {
      final value1 = SmallSerial(1);
      final value2 = SmallInteger(-1);
      final operation = value1 - value2;
    ...
    test('should return BigSerial as result when subtracting BigSerial', () {
    ...
    test('should return Serial as result when subtracting Serial', () {
      final value1 = SmallSerial(2);
    ...
    test('should return SmallSerial as result when subtracting SmallSerial',
        () {
      final value1 = SmallSerial(2);
    ...
    test('should return SmallSerial as result when subtracting int primitive',
        () {
      final value1 = SmallSerial(2);
      const value2 = 1;
      final operation = value1 - value2;
    ...
    test(
        'should return SmallSerial as result when subtracting negative int primitive',
        () {
      final value1 = SmallSerial(1);
      const value2 = -1;
      final operation = value1 - value2;
