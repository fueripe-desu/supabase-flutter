...
void main() {
  group('positive values', () {
    ...
    test('should return 0 if scale is not given', () {
      final numeric = Decimal(value: '1.5', precision: 5);
      expect(numeric.digitsAfter, 0);
    });

    test('should return the implementation limit if Decimal is unconstrained',
        () {
      final numeric = Decimal(value: '1.5');
      expect(numeric.digitsAfter, Decimal.maxDigitsAfter);
    });

    test('should return 0 if scale is negative', () {
      final numeric = Decimal(value: '1.5', precision: 5, scale: -2);
      expect(numeric.digitsAfter, 0);
    });

    test('should return 0 if value is int', () {
      final numeric = Decimal(value: '43', precision: 2, scale: 0);
      expect(numeric.digitsAfter, 0);
    });

    test('should return the scale if value is fractional', () {
      final numeric = Decimal(value: '0.3', precision: 1, scale: 1);
