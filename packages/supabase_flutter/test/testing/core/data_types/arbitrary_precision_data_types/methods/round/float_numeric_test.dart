...
void main() {
  ...
  test('should round up if positive value has fractional part greater than .5',
      () {
    final value = Numeric(value: '3.7', precision: 2, scale: 1);
    final expected = Numeric(value: '4.0', precision: 2, scale: 1);
    final operation = value.round();
    expect(operation.identicalTo(expected), true);
  });
  ...
  test('should round down if positive value has fractional part less than .5',
      () {
    final value = Numeric(value: '3.2', precision: 2, scale: 1);
  ...
  test('should work correctly with positive values very close to zero', () {
    final value = Numeric(value: '0.49999999999999', precision: 15, scale: 14);
  ...
  test('should round up if positive value is slightly above .5', () {
    final value = Numeric(value: '2.50000000000001', precision: 15, scale: 14);
    final expected = Numeric(
  ...
  test('should round down if positive value is slightly below .5', () {
    final value = Numeric(value: '2.49999999999999', precision: 15, scale: 14);
    final expected = Numeric(
  ...
  test('should work correctly with very large numbers', () {
    final value = Numeric(value: '1e37', precision: 38, scale: 0);
    final expected = Numeric(value: '1e37', precision: 38, scale: 0);
    final operation = value.round();
    expect(operation.identicalTo(expected), true);
  });

  test('should work correctly with very small numbers', () {
    final value = Numeric(value: '-1e307', precision: 308, scale: 0);
    final expected = Numeric(value: '-1e307', precision: 308, scale: 0);
    final operation = value.round();
    expect(operation.identicalTo(expected), true);
  });
  ...
  test('should round with precision even complex floating point', () {
    ...
    final expected = Numeric(value: '123456790.0', precision: 18, scale: 9);
    final operation = value.round();
    expect(operation.identicalTo(expected), true);
  });
}
